                    ...
                    <a href='<?php echo $this->Html->url( array( 'controller'=>'articlePages', 'action'=>'view', 'slug1' => $article['Article']['id'], 'slug2' => $article['Article']['slug'], 'slug3' => '1' )) ?>' title='Aller à l&apos;article <?php echo $article['Article']['title'] ?>' class='image-box__link' itemprop='url'>
                        <?php
                            if( $article['Article']['thumbnail'] ) {
                                echo $this->Html->image('articlesThumbnails/' . $article['Article']['id'] . '/small_' . $article['Article']['thumbnail'], array('alt' => 'Article ' . $article['Article']['title'], 'srcset' => $this->webroot . 'img/articlesThumbnails/' . $article['Article']['id'] . '/small_' . $article['Article']['thumbnail'] . ' 1x, ' . $this->webroot . 'img/articlesThumbnails/' . $article['Article']['id'] . '/smallHR_' . $article['Article']['thumbnail'] . ' 2x', 'width' => '282', 'height' => '175', 'class' => 'image-box__image', 'itemprop' => 'image'));
                            }
                            else {
                                echo $this->Html->image('articlesThumbnails/noThumbnailSmall.png', array('alt' => 'Couverture de substitution', 'srcset' => $this->webroot . 'img/articlesThumbnails/small_noThumbnail.png 1x, ' . $this->webroot . 'img/articlesThumbnails/smallHR_noThumbnail.png 2x', 'width' => '282', 'height' => '175', 'class' => 'image-box__image', 'itemprop' => 'image'));
                            }
                        ?>
